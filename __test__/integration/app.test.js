...
describe('Thunderdome App', () => {
  ...
  describe('when on a Battle page as leader', () => {
    ...
    it('should have Delete Battle Button', async () => {
      await expect(page).toMatchElement('button', { text: 'Delete Battle' })
    })

    it('should have locked voting cards', async () => {
      await expect(page).toMatchElement('[data-testId="pointCard"][data-locked="true"]')
    })

    it('should have Unpointed and Pointed tabs count as 0', async () => {
      await expect(page).toMatchElement('button', { text: 'Unpointed (0)' })
    ...
    describe('when clicking the Add Plan button', () => {
      ...
      it('should then submit the form', async () => {
        await expect(page).toClick('form[name="addPlan"] button[type="submit"]')
      })

      it('should display the new Plan', async () => {
        await expect(page).toMatchElement('[data-testId="battlePlanName"]', { text: 'Drink Ale' })
      })

      it('should display control buttons', async () => {
        await expect(page).toMatchElement('[data-testId="battlePlan"][data-planName="Drink Ale"] button', { text: 'Delete' })
        await expect(page).toMatchElement('[data-testId="battlePlan"][data-planName="Drink Ale"] button', { text: 'Edit' })
        await expect(page).toMatchElement('[data-testId="battlePlan"][data-planName="Drink Ale"] button', { text: 'Activate' })
      })

      it('should have Unpointed tab count as 1', async () => {
        await expect(page).toMatchElement('button', { text: 'Unpointed (1)' })
      })
    })
    ...
    describe('when clicking the Activate Plan button', () => {
      it('should enable the Point Cards', async () => {
        ...
        await expect(page).toMatchElement('[data-testId="pointCard"][data-locked="false"]')
      })

      it('should enable the Voting Controls', async () => {
        await expect(page).toMatchElement('[data-testId="votingControls"]')
      })
    ...
    describe('when clicking a point card', () => {
      it('should activate that point card', async () => {
        await expect(page).toClick('[data-testId="pointCard"][data-point="1"]')
        await expect(page).toMatchElement('[data-testId="pointCard"][data-point="1"][data-active="true"]')
      })

      it('should flag the warrior as voted', async () => {
        await expect(page).toMatchElement('[data-testId="warriorCard"][data-warriorName="Thor"] [data-icon="vote-yea"]')
      })
    })
    ...
    describe('when clicking the Finish Voting', () => {
      ...
      it('should display the point count on the card(s) and lock them', async () => {
        await expect(page).toMatchElement('[data-testId="pointCard"][data-point="1"][data-locked="true"]')
        await expect(page).toMatchElement('[data-testId="pointCard"][data-point="1"] [data-testId="pointCardCount"]', { test: '1' })
      })

      it('should display each warriors voted points', async () => {
        await expect(page).toMatchElement('[data-testId="warriorCard"][data-warriorName="Thor"] [data-testId="warriorPoints"]', { text: '1' })
      })
    ...
    describe('when filling out the final points form', () => {
      ...
      it('should submit the Final Points form', async () => {
        await expect(page).toClick('form[name="savePlanPoints"] button[type="submit"]')
      })

      it('should update plan to Pointed Tab and its count', async () => {
        await expect(page).toMatchElement('button', { text: 'Pointed (1)' })
      })
    })
