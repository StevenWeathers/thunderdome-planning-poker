<script lang="ts">
  ...
  import { AlertTriangle, Mail, Info, CheckCircle, Send, ExternalLink, Sparkles, Code, Lock, BugIcon } from 'lucide-svelte';
  ...
  function submitSupportTicket() {
    ...
    xfetch(`/api/users/${$user.id}/support-ticket`, { body: {
      inquiry,
      fullName,
      email,
    }})
...
<PageLayout>
  <div class="max-w-8xl mx-auto px-4 py-12 text-gray-900 dark:text-gray-100 relative">
    ...
    <div class="text-center mb-16">
      <h1 class="text-5xl lg:text-7xl font-bold font-rajdhani dark:text-white uppercase tracking-wider mb-4 bg-gradient-to-r from-blue-600 via-purple-600 to-emerald-600 bg-clip-text text-transparent">
        {$LL.support()}
      </h1>
    ...
    <div class="grid lg:grid-cols-2 gap-8">
      <!-- Bug Report Section -->
      <div class="relative">
        <div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 dark:border-gray-700/50 p-8">
          <div class="flex items-start space-x-4 mb-6">
            ...
            <div>
              <h2 class="text-2xl lg:text-3xl font-bold font-rajdhani dark:text-white mb-2">
                Found a Bug?
              </h2>
          ...
          <div class="space-y-6">
            ...
            >
              ...
              <div class="relative flex items-center gap-3">
                <div class="p-1 rounded-lg bg-gray-100 dark:bg-gray-800 group-hover:bg-gray-200 dark:group-hover:bg-gray-700 transition-colors duration-300">
                  <GithubIcon width={24} class="transform group-hover:scale-110 transition-transform duration-300" />
                </div>
                <span class="group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors duration-300">Open GitHub Issues</span>
                <ExternalLink class="w-6 h-6 text-gray-500 group-hover:text-blue-500 transform group-hover:translate-x-1 group-hover:-translate-y-1 transition-all duration-300" />
              </div>
            </a>
            
            <div class="flex items-center justify-between p-4 bg-emerald-50 dark:bg-emerald-900/30 rounded-xl border border-emerald-200 dark:border-emerald-700">
      ...
      <div class="group relative">
        <div class="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/20 dark:border-gray-700/50 p-8">
          <div class="flex items-start space-x-4 mb-8">
            <div class="w-14 h-14 bg-gradient-to-br from-blue-500 to-emerald-500 rounded-2xl flex items-center justify-center shadow-lg">
              <Mail class="w-8 h-8 text-white" />
            </div>
            <div>
              <h2 class="text-2xl lg:text-3xl font-bold font-rajdhani dark:text-white uppercase mb-2">
                Contact Us
              </h2>
              <p class="text-gray-600 dark:text-gray-300">
          ...
          {#if !$user.name}
            <div class="text-center py-16">
              <div class="w-20 h-20 bg-gradient-to-br from-amber-400 to-orange-500 rounded-full flex items-center justify-center mx-auto shadow-xl mb-6">
                <Lock class="w-10 h-10 text-white" />
              </div>
              <h3 class="text-2xl font-bold text-gray-900 dark:text-white mb-3">Login Required</h3>
              <p class="text-gray-600 dark:text-gray-400 text-lg">Please log in to submit a support ticket and get personalized assistance.</p>
            </div>
          {:else if submitted}
            <div class="text-center py-16">
              <div class="w-20 h-20 bg-gradient-to-br from-emerald-400 to-green-500 rounded-full flex items-center justify-center mx-auto shadow-xl mb-6">
                <CheckCircle class="w-10 h-10 text-white" />
              </div>
              <h3 class="text-2xl font-bold text-emerald-800 dark:text-emerald-200 mb-3">Success!</h3>
              <p class="text-emerald-600 dark:text-emerald-400 text-lg">Thank you for your submission! We'll get back to you within 24 hours.</p>
            </div>
          {:else}
            <form
            ...
            >
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                  <label 
                    for="fullName" 
                    class="block font-bold text-gray-700 dark:text-gray-300 mb-4"
                  >
              ...
              <div class="space-y-2">
                <label 
                  for="inquiry" 
                  class="block font-bold text-gray-700 dark:text-gray-300 mb-2"
                >
              ...
              <div class="pt-6 flex justify-end">
                ...
                >
                  ...
                  {#if isSubmitting}
                    <div class="absolute inset-0 bg-gradient-to-r from-gray-400 via-gray-500 to-gray-600 flex items-center justify-center">
                      <div class="w-6 h-6 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
                    </div>
                  {/if}
                  
                  <!-- Dynamic background effects -->
                  <div class="absolute inset-0 bg-gradient-to-r from-blue-400/30 via-purple-400/30 to-emerald-400/30 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
                  
                  <!-- Content -->
                  <div class="relative flex items-center {isSubmitting ? 'opacity-0' : 'opacity-100'} transition-opacity duration-300">
                  ...
                  <div class="absolute inset-0 bg-gradient-to-r from-emerald-500 via-green-500 to-emerald-600 opacity-0 flex items-center justify-center transform scale-0 transition-all duration-500">
                    <CheckCircle class="w-6 h-6 text-white" />
                  </div>
                  
                  <!-- Enhanced shine effect -->
                  <div class="absolute inset-0 -top-2 -bottom-2 bg-gradient-to-r from-transparent via-white/25 to-transparent skew-x-12 -translate-x-full group-hover:translate-x-full transition-transform duration-1000 ease-out"></div>
                </button>
              </div>
            </form>
          {/if}
    ...
    <div class="mt-20 text-center">
      <div class="inline-flex items-center gap-2 px-6 py-3 bg-white/60 dark:bg-gray-800/60 backdrop-blur-xl rounded-full border border-white/20 dark:border-gray-700/50 shadow-lg">
        <div class="w-2 h-2 bg-green-500 rounded-full"></div>
        <span class="font-medium text-gray-600 dark:text-gray-400">We typically respond within 24 hours, thank you for your patience!</span>
      </div>
    </div>
  </div>
</PageLayout>