...
<Modal {closeModal} ariaLabel={$LL.modalStoryboardPersonas()}>
  <div class="persona-list pt-6">
      ...
      <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-4">
        <form onsubmit={handleAddSubmit} class="space-y-3">
          <div>
            ...
            <TextInput id="add-name" bind:value={addForm.name} placeholder="Enter persona name" autofocus />
          </div>
          <div>
            <label for="add-role" class="block text-gray-700 dark:text-gray-300 mb-1">Role</label>
            <TextInput id="add-role" bind:value={addForm.role} placeholder="Enter role (optional)" />
          </div>
          <div>
          ...
          <div class="flex justify-end space-x-2 mt-4">
            ...
            <SolidButton type="submit" disabled={!addForm.name.trim()}>Add Persona</SolidButton>
          </div>
        </form>
      </div>
    {/if}
    ...
    <div class="space-y-3">
      {#each personas as persona (persona.id)}
        <div
          class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-2 lg:p-4 hover:border-gray-300 dark:hover:border-gray-600 transition-colors"
        >
          {#if editingId === persona.id && isFacilitator}
            ...
            <form onsubmit={e => handleEditSubmit(e, persona)} class="space-y-3">
              <div>
                ...
                <TextInput id="edit-name-{persona.id}" bind:value={editForm.name} autofocus />
              </div>
              <div>
                <label for="edit-role-{persona.id}" class="block text-gray-700 dark:text-gray-300 mb-1">Role</label>
                <TextInput id="edit-role-{persona.id}" bind:value={editForm.role} />
              </div>
              <div>
                <label for="edit-description-{persona.id}" class="block text-gray-700 dark:text-gray-300 mb-1"
                  >Description</label
                >
                <textarea
              ...
              <div class="flex justify-end space-x-2">
                ...
                <SolidButton type="submit" disabled={!editForm.name.trim()}>Save</SolidButton>
              </div>
            </form>
          {:else}
            <!-- Display Mode -->
            <div class="flex justify-between items-center">
              <div class="flex-1">
                <div class="flex items-center space-x-2 mb-1">
                  ...
                  {#if persona.role}
                    <span
                      class="inline-flex items-center px-2 py-0.5 rounded text-xs bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-200"
                    >
                      {persona.role}
                    </span>
                  {/if}
      ...
      {:else}
        <div class="py-8 text-gray-600 dark:text-gray-300 font-bold">
          ...
          <p class="rounded-lg text-left bg-gray-200 dark:bg-gray-700 p-3">
            User personas in agile story mapping represent different user types whose unique goals and needs help
            organize story columns by ensuring each vertical slice addresses specific user journeys and priorities.
          </p>
        </div>
      {/each}
    </div>
