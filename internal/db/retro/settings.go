...
func (s *Service) CreateSettings(ctx context.Context, settings *thunderdome.RetroSettings) error {
...
func (s *Service) UpdateSettings(ctx context.Context, settings *thunderdome.RetroSettings) error {
	_, err := s.DB.ExecContext(ctx, `
		...
		settings.TemplateID, settings.JoinCode, settings.FacilitatorCode, settings.ID)
	return err
}
