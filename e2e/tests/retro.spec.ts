...
test.describe('Retro page', () => {
    ...
    test.beforeAll(async ({registeredPage, verifiedPage, adminPage}) => {
        ...
        retroCancelDelete = await registeredPage.createRetro({...commonRetro});
    ...
    test('delete retro confirmation confirm deletes retro and redirects to retros page', async ({registeredPage}) => {
        const bp = new RetroPage(registeredPage.page);
