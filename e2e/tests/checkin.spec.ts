...
test.describe('Team Checkin page', () => {
    ...
    test.describe('Registered user', () => {
        ...
        test('loads organization team checkin page', async ({registeredPage}) => {
            ...
            await expect(teamPage.page.locator('a', {hasText: testTeamName})).toBeVisible();
            await expect(teamPage.page.locator('h1')).toHaveText('Check In');
        })

        test('loads department team checkin page', async ({registeredPage}) => {
            const testTeamName = 'E2E TEST TEAM';
        ...
        test('can checkin once today', async ({registeredPage}) => {
            ...
            await expect(teamPage.page.locator('[data-testid="checkin-yesterday"]'))
                .toHaveText("Yesterday I fixed bugs");
            await expect(teamPage.page.locator('[data-testid="checkin-today"]'))
            ...
            await expect(teamPage.page.locator('[data-testid="checkin-blockers"]'))
                .toHaveText("Blocked by procrastination");
            await expect(teamPage.page.locator('[data-testid="checkin-discuss"]'))
