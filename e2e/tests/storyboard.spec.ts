...
test.describe('Storyboard page', () => {
    ...
    test.beforeAll(async ({registeredPage, verifiedPage, adminPage}) => {
        const commonStoryboard = {
            ...
            storyboardFacilitators: [
                `${adminPage.user.email}`
            ]
        };
        storyboard = await registeredPage.createStoryboard({...commonStoryboard});
        storyboardLeave = await verifiedPage.createStoryboard({...commonStoryboard});
        storyboardCancelDelete = await registeredPage.createStoryboard({...commonStoryboard});
        storyboardDelete = await registeredPage.createStoryboard({...commonStoryboard});
    });
    ...
    test('delete storyboard confirmation cancel does not delete storyboard', async ({registeredPage}) => {
        const bp = new StoryboardPage(registeredPage.page);
    ...
    test('delete storyboard confirmation confirm deletes storyboard and redirects to storyboards page', async ({registeredPage}) => {
        const bp = new StoryboardPage(registeredPage.page);
