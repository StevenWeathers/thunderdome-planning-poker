...
test.describe("Organization Team API", { tag: ["@api", "@team", "@organization"] }, () => {
  test.describe("POST /organizations/{orgId}/teams", () => {
    test("Org Admin can create a new team in any department of their organization", async ({
      ...
      const dept = await testDatabase.seeder.createDepartment("Dept for New Team", org.id);
      const newTeamName = "New Team Created by Org Admin";
      const response = await orgAdminApiUser.context.post(`organizations/${org.id}/teams`, {
  ...
  test.describe("GET /teams/{teamId}", () => {
    test("Team Admin can view team details", async ({ orgTeamApiUser, orgTeamAdminApiUser }) => {
      const team = orgTeamApiUser.user.orgTeams[0];
      const getResponse = await orgTeamAdminApiUser.context.get(`teams/${team.id}`);
  ...
  test.describe("PUT /teams/{teamId}", () => {
    ...
    test("Org Admin can update any team in their organization", async ({
      ...
      const org = orgAdminApiUser.user.orgs[0];
      const dept = await testDatabase.seeder.createDepartment("Dept for Team Update", org.id);
      const team = await testDatabase.seeder.createOrgTeam("Team to Update", org.id, dept.id);
  ...
  test.describe("DELETE /teams/{teamId}", () => {
    ...
    test("Org Admin can delete any team in their organization", async ({
      ...
      const org = orgAdminApiUser.user.orgs[0];
      const dept = await testDatabase.seeder.createDepartment("Dept for Team Deletion", org.id);
      const team = await testDatabase.seeder.createOrgTeam("Team to Delete", org.id, dept.id);
      const response = await orgAdminApiUser.context.delete(`teams/${team.id}`);
      expect(response.ok()).toBeTruthy();
      expect(response.status()).toBe(200);
    });
