...
test.describe("Organization Department API", { tag: ["@api", "@department"] }, () => {
  test.describe("GET /organizations/{orgId}/departments", () => {
    test("returns departments list for org member", async ({ request, orgTeamApiUser }) => {
      const org = orgTeamApiUser.user.orgs[0];
      const response = await orgTeamApiUser.context.get(`organizations/${org.id}/departments`);
    ...
    test("returns departments list for org admin", async ({ request, orgOwnerApiUser }) => {
      const org = orgOwnerApiUser.user.orgs[0];
      const response = await orgOwnerApiUser.context.get(`organizations/${org.id}/departments`);
      expect(response.ok()).toBeTruthy();
      expect(response.status()).toBe(200);
      const departments = await response.json();
      expect(Array.isArray(departments.data)).toBeTruthy();
    ...
    test("returns departments list for global admin", async ({
      ...
      const response = await adminApiUser.context.get(`organizations/${org.id}/departments`);
      expect(response.ok()).toBeTruthy();
      expect(response.status()).toBe(200);
      const departments = await response.json();
      expect(Array.isArray(departments.data)).toBeTruthy();
    ...
    test("returns 403 Forbidden for non-org member", async ({
      ...
      const response = await registeredApiUser.context.get(`organizations/${org.id}/departments`);
    ...
    test("returns empty array when no departments exist", async ({
      ...
      const response = await orgOwnerApiUser.context.get(`organizations/${newOrg.id}/departments`);
      expect(response.ok()).toBeTruthy();
      expect(response.status()).toBe(200);
      const departments = await response.json();
      expect(Array.isArray(departments.data)).toBeTruthy();
  ...
  test.describe("POST /organizations/{orgId}/departments", () => {
    test("returns 200 for organization admin", async ({ request, orgOwnerApiUser }) => {
      const org = orgOwnerApiUser.user.orgs[0];
      const departmentName = "Test API Create Department Org Admin";
      const response = await orgOwnerApiUser.context.post(`organizations/${org.id}/departments`, {
    ...
    test("returns 403 for organization member", async ({
      ...
      const response = await orgTeamApiUser.context.post(`organizations/${org.id}/departments`, {
        data: { name: departmentName },
      });
      expect(response.ok()).toBeFalsy();
      expect(response.status()).toBe(403);
    ...
    test("returns 200 for global admin", async ({ request, orgOwnerApiUser, adminApiUser }) => {
      const org = orgOwnerApiUser.user.orgs[0];
      const departmentName = "Test API Create Department Global Admin";
      const response = await adminApiUser.context.post(`organizations/${org.id}/departments`, {
        data: { name: departmentName },
      });
      expect(response.ok()).toBeTruthy();
      const department = await response.json();
  ...
  test.describe("PUT /organization/{orgId}/departments/{deptId}", () => {
    test("Org Admin can update any department in their organization", async ({
      ...
      const dept = await testDatabase.seeder.createDepartment("Dept to Update", org.id);
  ...
  test.describe("DELETE /api/organizations/{orgId}/departments/{deptId}", () => {
    test("returns 200 for org admin", async ({ request, orgOwnerApiUser, testDatabase }) => {
      const org = orgOwnerApiUser.user.orgs[0];
      const dept = await testDatabase.seeder.createDepartment("deptToDeleteOrgOwner", org.id);
      ...
      const confirmDeptDeleted = await testDatabase.seeder.getDepartmentById(dept.id);
    ...
    test(
      ...
      async ({ request, orgOwnerApiUser, adminApiUser, testDatabase }) => {
        const org = orgOwnerApiUser.user.orgs[0];
        const dept = await testDatabase.seeder.createDepartment("deptToDeleteGlobalAdmin", org.id);
    ...
    test("returns 403 Forbidden for non org admin", async ({ request, deptTeamApiUser }) => {
      const org = deptTeamApiUser.user.orgs[0];
      const dept = deptTeamApiUser.user.depts[0];
