...
class RegisteredPage {
    ...
    public async createTeam(name: string) {
        const t = await this.page.request.post(`/api/users/${this.user.id}/teams`, {
            data: {
                name: name
            }
        });
    ...
    public async createDepartmentTeam(orgId, deptId, name) {
        const dt = await this.page.request.post(`/api/organizations/${orgId}/departments/${deptId}/teams`, {
            data: {
                name: name,
            }
        });
...
class VerifiedPage {
    ...
    static async create(browser: Browser) {
        const context = await browser.newContext({storageState: 'storage/verifiedStorageState.json'});
        const page = await context.newPage();
