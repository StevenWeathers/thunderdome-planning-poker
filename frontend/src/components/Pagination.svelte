<script>
  ...
  <div class="flex text-gray-700 text-lg">
    ...
    <div
      class="flex h-12 font-medium">
      {#each arr_pages as i}
        <div
          class="w-12 sm:flex justify-center items-center hidden select-none
          cursor-pointer leading-5 transition duration-150 ease-in {i == current ? `border-t-2 border-indigo-600 ` : 'border-t-2 border-white'}
          "
      ...
      {/each}
      <div
        class="w-12 h-12 sm:hidden flex justify-center select-none items-center
        cursor-pointer leading-5 transition duration-150 ease-in border-t-2 border-indigo-600">
        {current}
      </div>
    </div>
    <div
