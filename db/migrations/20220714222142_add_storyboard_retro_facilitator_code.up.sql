ALTER TABLE retro ADD COLUMN facilitator_code TEXT;
ALTER TABLE retro ALTER COLUMN join_code TYPE TEXT;
ALTER TABLE storyboard ADD COLUMN facilitator_code TEXT;
ALTER TABLE storyboard ALTER COLUMN join_code TYPE TEXT;
ALTER TABLE battles ALTER COLUMN join_code TYPE TEXT;
ALTER TABLE battles ALTER COLUMN leader_code TYPE TEXT;
...
BEGIN
    UPDATE retro
    SET name = retroName, join_code = joinCode, facilitator_code = facilitatorCode, max_votes = maxVotes,
...
DROP FUNCTION create_storyboard(ownerId UUID, storyboardName VARCHAR(256), joinCode VARCHAR(128));
...
-- Edit a Storyboard
DROP PROCEDURE edit_storyboard(storyboardId UUID, storyboardName VARCHAR(256), joinCode VARCHAR(128));
CREATE PROCEDURE edit_storyboard(storyboardId UUID, storyboardName VARCHAR(256), joinCode TEXT, facilitatorCode TEXT)
LANGUAGE plpgsql AS $$
BEGIN
    UPDATE storyboard
        SET name = storyboardName, join_code = joinCode, facilitator_code = facilitatorCode, updated_date = NOW()
...
CREATE FUNCTION get_retros_by_user(userId UUID) RETURNS table (
    id UUID, name VARCHAR(256), owner_id UUID, format VARCHAR(32), phase VARCHAR(16), created_date timestamptz, updated_date timestamptz
) AS $$
BEGIN
    RETURN QUERY
